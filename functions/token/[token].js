...
function applyAllowedUpdates(existing, body, { allowSender = false } = {}){
  ...
  if (allowSender){
    ...
    if ('from_line' in body){
      next.from_line = (body.from_line == null) ? null : String(body.from_line).slice(0, 100);
    }
    if ('card_style' in body){
      const s = String(body.card_style || '').slice(0, 64);
      if (s) next.card_style = s;
    }
    if ('scratch_shape' in body){
      const VALID_SHAPES = ['heart','circle','star','hexagon','square','grunge'];
